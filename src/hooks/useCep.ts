...
import { EnderecoProps, FormInputEndEsp } from "../types/especialistaTipos";
import { useCallback, useEffect } from "react";
import { supabase } from "../libs/supabase";

const useCep = () =>{
    
    const { register, handleSubmit, formState:{errors}, setValue, watch } = useForm<FormInputEndEsp>({
        resolver: zodResolver(schemaCadastroEnderecoEsp),
        defaultValues: {
          endereco: {
            cep: "",
    ...
    const aoSubmeter = async (dados: FormInputEndEsp) => {
        await supabase.storage.from("react-form").upload(dados.endereco.avatar.name, dados.endereco.avatar);
        console.log(dados);
    };
